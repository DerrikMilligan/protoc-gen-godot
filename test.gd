...
class TestProtobufEncoder:
  ...
  static func test_message_encoding():
    ...
    }).encode() == PackedByteArray([0x8A, 0x02, 0x15, 0xC2, 0x02, 0x04, 0x71, 0x77, 0x65, 0x72, 0xCA, 0x02, 0x04, 0x61, 0x73, 0x64, 0x66, 0xD2, 0x02, 0x04, 0x7A, 0x78, 0x63, 0x76]))
    assert(messages.TestMessage.new({ 'repeated_string': [ 'qwer', 'asdf', 'zxcv' ] }).encode() == PackedByteArray([0x92, 0x03, 0x04, 0x71, 0x77, 0x65, 0x72, 0x92, 0x03, 0x04, 0x61, 0x73, 0x64, 0x66, 0x92, 0x03, 0x04, 0x7A, 0x78, 0x63, 0x76]))

    assert(messages.TestMessage.new({
      'some_int32': {
        'case': 'oneof_sub_message',
        'value': {
          'sequential_string1': 'Sequential 1',
          'repeated_string': [ 'qwer', 'asdf', 'zxcv' ],
        }
      }
    }).encode() == PackedByteArray([0x8A, 0x02, 0x24, 0xC2, 0x02, 0x0C, 0x53, 0x65, 0x71, 0x75, 0x65, 0x6E, 0x74, 0x69, 0x61, 0x6C, 0x20, 0x31, 0x92, 0x03, 0x04, 0x71, 0x77, 0x65, 0x72, 0x92, 0x03, 0x04, 0x61, 0x73, 0x64, 0x66, 0x92, 0x03, 0x04, 0x7A, 0x78, 0x63, 0x76]))

    assert(messages.TestMessage.new({
      'repeated_submessage': [
        messages.SubMessage.new({
          'sequential_string1': 'Sequential 1',
          'repeated_string': [ 'qwer', 'asdf', 'zxcv' ],
        }),
        messages.SubMessage.new({
          'sequential_string1': 'Sequential 2',
          'repeated_string': [ 'zxcv', 'asdf', 'qwer' ],
        }),
      ]
    }).encode() == PackedByteArray([0x9A, 0x03, 0x24, 0xC2, 0x02, 0x0C, 0x53, 0x65, 0x71, 0x75, 0x65, 0x6E, 0x74, 0x69, 0x61, 0x6C, 0x20, 0x31, 0x92, 0x03, 0x04, 0x71, 0x77, 0x65, 0x72, 0x92, 0x03, 0x04, 0x61, 0x73, 0x64, 0x66, 0x92, 0x03, 0x04, 0x7A, 0x78, 0x63, 0x76, 0x9A, 0x03, 0x24, 0xC2, 0x02, 0x0C, 0x53, 0x65, 0x71, 0x75, 0x65, 0x6E, 0x74, 0x69, 0x61, 0x6C, 0x20, 0x32, 0x92, 0x03, 0x04, 0x7A, 0x78, 0x63, 0x76, 0x92, 0x03, 0x04, 0x61, 0x73, 0x64, 0x66, 0x92, 0x03, 0x04, 0x71, 0x77, 0x65, 0x72]))
  ...
  static func test_message_decoding():
    ...
    var sequential_strings = messages.TestMessage.from_bytes([0xC2, 0x02, 0x04, 0x71, 0x77, 0x65, 0x72, 0xCA, 0x02, 0x04, 0x61, 0x73, 0x64, 0x66, 0xD2, 0x02, 0x04, 0x7A, 0x78, 0x63, 0x76])
    assert(sequential_strings.encode() == PackedByteArray([0xC2, 0x02, 0x04, 0x71, 0x77, 0x65, 0x72, 0xCA, 0x02, 0x04, 0x61, 0x73, 0x64, 0x66, 0xD2, 0x02, 0x04, 0x7A, 0x78, 0x63, 0x76]))
    assert(sequential_strings.fields['sequential_string1'].get_value() == 'qwer')
    assert(sequential_strings.fields['sequential_string2'].get_value() == 'asdf')
    assert(sequential_strings.fields['sequential_string3'].get_value() == 'zxcv')

    var sub_sequential_strings = messages.TestMessage.from_bytes([0x8A, 0x02, 0x15, 0xC2, 0x02, 0x04, 0x71, 0x77, 0x65, 0x72, 0xCA, 0x02, 0x04, 0x61, 0x73, 0x64, 0x66, 0xD2, 0x02, 0x04, 0x7A, 0x78, 0x63, 0x76])
    assert(sub_sequential_strings.encode() == PackedByteArray([0x8A, 0x02, 0x15, 0xC2, 0x02, 0x04, 0x71, 0x77, 0x65, 0x72, 0xCA, 0x02, 0x04, 0x61, 0x73, 0x64, 0x66, 0xD2, 0x02, 0x04, 0x7A, 0x78, 0x63, 0x76]))
    var sub_message = sub_sequential_strings.get_some_int32()
    assert(sub_message.get_sequential_string1() == 'qwer')
    assert(sub_message.get_sequential_string2() == 'asdf')
    assert(sub_message.get_sequential_string3() == 'zxcv')

    var repeated_strings = messages.TestMessage.from_bytes([
      0x92, 0x03, 0x04, 0x71, 0x77, 0x65, 0x72,
      0x92, 0x03, 0x04, 0x61, 0x73, 0x64, 0x66,
      0x92, 0x03, 0x04, 0x7A, 0x78, 0x63, 0x76
    ])
    assert(repeated_strings.encode() == PackedByteArray([0x92, 0x03, 0x04, 0x71, 0x77, 0x65, 0x72, 0x92, 0x03, 0x04, 0x61, 0x73, 0x64, 0x66, 0x92, 0x03, 0x04, 0x7A, 0x78, 0x63, 0x76]))
    assert(repeated_strings.get_repeated_string() == [ 'qwer', 'asdf', 'zxcv' ])

    var nested_oneof_repeated_strings = messages.TestMessage.from_bytes([0x8A, 0x02, 0x24, 0xC2, 0x02, 0x0C, 0x53, 0x65, 0x71, 0x75, 0x65, 0x6E, 0x74, 0x69, 0x61, 0x6C, 0x20, 0x31, 0x92, 0x03, 0x04, 0x71, 0x77, 0x65, 0x72, 0x92, 0x03, 0x04, 0x61, 0x73, 0x64, 0x66, 0x92, 0x03, 0x04, 0x7A, 0x78, 0x63, 0x76])
    assert(nested_oneof_repeated_strings.get_some_int32().get_sequential_string1() == 'Sequential 1')
    assert(nested_oneof_repeated_strings.get_some_int32().get_repeated_string() == [ 'qwer', 'asdf', 'zxcv'])

    var repeated_submessage = messages.TestMessage.from_bytes([0x9A, 0x03, 0x24, 0xC2, 0x02, 0x0C, 0x53, 0x65, 0x71, 0x75, 0x65, 0x6E, 0x74, 0x69, 0x61, 0x6C, 0x20, 0x31, 0x92, 0x03, 0x04, 0x71, 0x77, 0x65, 0x72, 0x92, 0x03, 0x04, 0x61, 0x73, 0x64, 0x66, 0x92, 0x03, 0x04, 0x7A, 0x78, 0x63, 0x76, 0x9A, 0x03, 0x24, 0xC2, 0x02, 0x0C, 0x53, 0x65, 0x71, 0x75, 0x65, 0x6E, 0x74, 0x69, 0x61, 0x6C, 0x20, 0x32, 0x92, 0x03, 0x04, 0x7A, 0x78, 0x63, 0x76, 0x92, 0x03, 0x04, 0x61, 0x73, 0x64, 0x66, 0x92, 0x03, 0x04, 0x71, 0x77, 0x65, 0x72]);
    var submessages = repeated_submessage.get_repeated_submessage()
    assert(submessages[0].get_sequential_string1() == 'Sequential 1')
    assert(submessages[1].get_sequential_string1() == 'Sequential 2')
    assert(submessages[0].get_repeated_string() == [ 'qwer', 'asdf', 'zxcv' ])
    assert(submessages[1].get_repeated_string() == [ 'zxcv', 'asdf', 'qwer' ])
